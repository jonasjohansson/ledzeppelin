...
Surfaces (**warm** greige — a tan/beige-tinted dark, R>G>B, ascending): `--bg → --field-bg → --panel → --panel-2 → --hover → --line → --line-2` (no accent tint on chrome; the warmth is baked into the anchors so the cool accent + category colours pop). Text (warm cream): `--text → --readout → --muted → --faint`. Accent: ONE **fixed** teal-mint `#3ecfa6` (no picker — pinned in palette code); variants `--accent-soft` (selected tint), `--accent-line` (selected border), `--accent-text` (text on tint), `--accent-dark` (glyph on a bright accent fill), `--accent-head` (muted-teal group-header bar).
