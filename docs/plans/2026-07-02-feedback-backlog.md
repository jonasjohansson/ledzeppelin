...
- **C2 (#4/#6) Popups everywhere.** Library, Mapping, **and Settings** open as sized popup windows (`window.open` with width/height — minimal chrome; truly chromeless in the installed app). Settings gets a **top-left header button**; remove the Settings tab from the left island. Reverses the earlier tabs decision — intentional. **DONE** — Settings initially shipped as a floating gear panel (the earlier deviation); it now opens as a real `/settings/` window (`src/ui/settings.js` panel + popout hooks, `BroadcastChannel('lz-settings')` sync, targeted `audioDevice`/`audioGain` merge in the main window; adopted edits are non-undoable, same rule as inventory merges).
