...
test('a daisy-chain run derives as a chain (members in data-flow order)', () => {
  const show = importKagora(preset);
  // Chains are DERIVED from shared (device, port) + wiring order (see chains.js):
  // sA-1 → sA-2 on brainA's output 1 reads back as one chain, in order.
