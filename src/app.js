...
// --- Screen record (⏺ rec): capture THIS site to video, then download it. Clicking
//     opens a format menu — WebM (full-res) or MP4 (lo-res, downscaled). We pass
//     preferCurrentTab so the browser pre-selects this tab (one click, "the site").
//     Stops on click again, or when you end sharing from the browser bar. ---
(() => {
  const recBtn = document.getElementById('rec-btn');
  if (!recBtn) return;
  let rec = null, chunks = [], srcStream = null, pumpRaf = 0;
  ...
  // mp4 = downscale to ~480p via a canvas pipe (smaller file, easy to share); webm
  // records the captured stream at full resolution.
  const MP4_MIMES = ['video/mp4;codecs=h264', 'video/mp4;codecs=avc1.42E01E', 'video/mp4'];
  const WEBM_MIMES = ['video/webm;codecs=vp9', 'video/webm;codecs=vp8', 'video/webm'];
  async function startRec(format) {
    try { srcStream = await navigator.mediaDevices.getDisplayMedia({ preferCurrentTab: true, video: { frameRate: 30 }, audio: false }); }
    catch { srcStream = null; return; }   // user cancelled the picker
    let recStream = srcStream, mime, ext, bitrate;
    if (format === 'mp4') {
      mime = MP4_MIMES.find((m) => MediaRecorder.isTypeSupported(m));
      ext = mime ? 'mp4' : 'webm';   // fall back to webm if this browser can't encode mp4
      if (!mime) mime = WEBM_MIMES.find((m) => MediaRecorder.isTypeSupported(m)) || 'video/webm';
      bitrate = 2.5e6;
      // Downscale: play the capture into a hidden <video>, draw it into a small
      // canvas each frame, and record THAT canvas's stream.
      const s = srcStream.getVideoTracks()[0].getSettings();
      const vw = s.width || 1280, vh = s.height || 720, k = Math.min(1, 854 / vw);
      const ow = Math.max(2, Math.round(vw * k)), oh = Math.max(2, Math.round(vh * k));
      const vid = Object.assign(document.createElement('video'), { srcObject: srcStream, muted: true, playsInline: true });
      try { await vid.play(); } catch { /* autoplay of a muted stream */ }
      const cnv = Object.assign(document.createElement('canvas'), { width: ow, height: oh });
      const cx = cnv.getContext('2d');
      const pump = () => { cx.drawImage(vid, 0, 0, ow, oh); pumpRaf = requestAnimationFrame(pump); };
      pump();
      recStream = cnv.captureStream(30);
    } else {
      mime = WEBM_MIMES.find((m) => MediaRecorder.isTypeSupported(m)) || 'video/webm';
      ext = 'webm'; bitrate = 8e6;
    }
    chunks = [];
    ...
    rec.onstop = () => {
      ...
      srcStream?.getTracks().forEach((t) => t.stop()); srcStream = null;
      const blob = new Blob(chunks, { type: mime });
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `ledzeppelin.${ext}`; a.click();
      setTimeout(() => URL.revokeObjectURL(a.href), 4000);
      rec = null; chunks = []; paint();
    };
    srcStream.getVideoTracks()[0].addEventListener('ended', () => { if (rec && rec.state !== 'inactive') rec.stop(); });   // "Stop sharing"
    rec.start();
    paint();
  }
  recBtn.addEventListener('click', (e) => {
    if (rec) { if (rec.state !== 'inactive') rec.stop(); return; }   // already recording → stop
    e.stopPropagation();
    openMenu(recBtn, menuList([
      { label: 'Record WebM', act: () => startRec('webm') },
      { label: 'Record MP4 (lo-res)', act: () => startRec('mp4') },
    ]));
  });
})();
